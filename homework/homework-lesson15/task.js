...
document.getElementById('futureTasks').addEventListener( 'click', showFutureTasks);
function showFutureTasks() {
    for (var i = 0; i < task.taskCollection.length; i++) {
        if(Math.floor(new Date() / 1000) < task.taskCollection[i].startTime) {
            
            var elem = document.getElementById("tasksList");
            elem.outerHTML += '<br>' + '<p id="del">' + task.taskCollection[i].describe + ' ' +
            '<button onclick="delTask()"> x </button>' + '</p>';
...
/* function getStartTime() {
    ...
    var defaultTime =  Math.floor(new Date()/1000)+(i+counter);
    counter +=2;
